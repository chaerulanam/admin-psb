...
class Users extends BaseController
{
    ...
    public function index()
    {
        ...
        $data = [
            'bagian' => $this->grupModel->findAll(),
            'profil' => $this->profilModel->where('user_id', user()->id)->get()->getRow(),
            'title_meta' => view('partials/title-meta', ['title' => 'Data Users']),
            'page_title' => view('partials/page-title', ['title' => 'Data Users', 'pagetitle' => 'Data Users']),
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($posts = $this->userModel
                ...
                foreach ($posts as $key) {
                    ...
                    $row[] = $key->email;
                    $row[] = $key->name;
                    if ($key->name == 'admin | superadmin' || $key->name == 'superadmin | admin') {
                        ...
                        <a href="javascript:void(0);" class="btn btn-outline-danger" id="button-hapus" data-id="' . $key->userid . '">
    ...
    public function ubah()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            } else if (!$this->userModel->save($data)) {
                $data = array('error' => 'Gagal Mengubah user');
            }

            $data = [
                'user_id' => user()->id,
                'pesan' => 'Mengubah User  ' . $this->request->getPost('username'),
            ];
            $this->logModel->save($data);

            $data = array('success' => 'Berhasil mengubah user  ' . $this->request->getPost('username'));
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->userModel->delete($id)) {
                $data = array('error' => 'Gagal Menghapus User');
                $data[$csrfname] = $csrfhash;
                return $this->response->setJSON($data);
            }
            ...
            $data = [
                'user_id' => user()->id,
                'pesan' => 'Menghapus User  ' . $user->username,
            ];

            $this->logModel->save($data);

            $data = array('success' => 'Berhasil Menghapus User');
            $data[$csrfname] = $csrfhash;
            return $this->response->setJSON($data);
        } else {
            throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
    ...
    public function import()
    {
        if ($this->request->isAJAX()) {
            ...
            foreach ($post as $key) {
                $data = [
                    'username' => $key[0],
                    'email' => $key[1] . '@alishlah.ponpes.id',
                    'password' => $key[2],
                ];
                ...
                $data = [
                    'user_id' => user()->id,
                    'pesan' => 'Menambahkan User Baru ' . $key[1] . '@alishlah.ponpes.id',
                ];
    ...
    public function attemptRegister()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'user_id' => user()->id,
                'pesan' => 'Menambahkan User Baru ' . $this->request->getPost('username'),
            ];
